...
# Oczekuj, aż kontener będzie uruchomiony
until docker logs $CONTAINER_NAME | grep -q "Done! You can now compile ESP-IDF projects."; do
...
if [ "$IS_LOCAL_BUILD" = true ]; then
  ...
  docker exec -i "$CONTAINER_NAME" /bin/bash -c "cd /usr/local/build && idf.py build"
else
  echo "Running build.sh script inside the container..."
  docker exec -i "$CONTAINER_NAME" /bin/bash -c "/usr/local/scripts/build.sh"
fi
